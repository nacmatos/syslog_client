...
Works by sending UDP messages to a remote syslog server.
The remote server must be configured to accept logs from the network.
...
class Syslog:
  ...
  def send(self, message, level):
    ...
    data = bytearray("<%d>%s %s %s" % (level + self.facility*8, dt.now().strftime('%b %d %H:%M:%S'), socket.gethostname(), message), 'utf-8')
